  ...
  <div class="match-edit-item">
    <div class="match-edit-header">
      <span class="badge" :class="match.completed ? 'badge-green' : 'badge-yellow'">
        {{ match.completed ? match.score1 + ':' + match.score2 : '未開始' }}
      </span>
      <button class="btn btn-danger btn-sm" @click="$emit('delete')">刪除</button>
    </div>

    <!-- Team selection -->
    <div class="score-input-row" style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">
      <select v-model.number="match.team1Id" class="form-control" style="flex:1;">
        <option :value="null">選擇隊伍</option>
        <option v-for="t in teams" :key="t.id" :value="t.id">{{ t.name }}</option>
      </select>
      <span style="color:var(--text-muted);font-weight:600;">VS</span>
      <select v-model.number="match.team2Id" class="form-control" style="flex:1;">
        <option :value="null">選擇隊伍</option>
        <option v-for="t in teams" :key="t.id" :value="t.id">{{ t.name }}</option>
      </select>
    </div>
    ...
    <div v-for="(d, di) in match.doubles" :key="di" class="doubles-edit-item">
      <div class="doubles-edit-label">第 {{ di + 1 }} 點</div>
      ...
      <div class="game-scores">
        <div v-for="g in 7" :key="g" class="game-score-pair">
    ...
    <button class="btn btn-success btn-sm" style="margin-top:8px;" @click="save">儲存成績</button>
  </div>
</template>

<script setup>
import { computed } from 'vue'
import { getTeamById } from '../lib/main-storage'
...
function save() {
  props.match.doubles.forEach(d => {
    ...
    d.score1 = s1
    d.score2 = s2
    d.completed = validGames.length > 0 && (s1 >= 4 || s2 >= 4)
  })
  emit('save')
}
</script>
